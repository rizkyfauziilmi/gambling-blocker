...
def test_submit_report(
    ...
    resp = client.post(
        "/report", json={"url": "https://example.com/game", "gambling_score": 0.95}
    )
...
def test_submit_report_blacklisted_url(client: TestClient) -> None:
    ...
    resp = client.post(
        "/report", json={"url": "https://blocked.com/game", "gambling_score": 0.9}
    )
...
def test_submit_report_whitelisted_url(client: TestClient) -> None:
    ...
    resp = client.post(
        "/report", json={"url": "https://allowed.com/game", "gambling_score": 0.9}
    )
    assert resp.status_code == 400
