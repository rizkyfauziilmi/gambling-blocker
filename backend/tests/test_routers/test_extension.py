...
def test_tamper_alert(client: TestClient) -> None:
    resp = client.post(
        "/extension/tamper-alert",
        json={"extension_id": "ext-1", "event_type": "failed_password"},
    )
    assert resp.status_code == 200
    assert resp.json()["ok"] is True
...
def test_setup_partner_email_sent(client: TestClient) -> None:
    with patch("routers.extension.send_partner_password", return_value=True):
        resp = client.post(
            "/extension/setup",
            json={"extension_id": "ext-1", "partner_email": "partner@example.com"},
        )
    assert resp.status_code == 200
...
def test_reset_password(client: TestClient) -> None:
    with patch("routers.extension.send_partner_password", return_value=True):
        client.post(
            "/extension/setup",
            json={"extension_id": "ext-1", "partner_email": "p@example.com"},
        )
...
def test_reset_password_not_found(client: TestClient) -> None:
    resp = client.post(
        "/extension/reset-password", json={"extension_id": "ext-unknown"}
    )
